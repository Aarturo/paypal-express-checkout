...
/**
 * Get the transaction datails, for instance when you need to check the pending status 
 * @param {string}   TransactionID unique identifier
 * @param {Function} callback      [description]
 */
Paypal.prototype.getTransactionDetails = function(TransactionID, callback) {
	var self = this;
	var params = self.params();
	...
	self.request(self.url, 'GET', params, function(err, data) {
		if (err) {
			callback(err, data);
			return;
		}
 ...
 * @param {Object} args
 */
Paypal.prototype.CreateRecurringPaymentsProfile = function(args, callback) {
	var self = this;
	var params = self.params();
	...
	params.PAYERID = args.PAYERID;   //Identifies the customer's account
	params.PROFILESTARTDATE = args.PROFILESTARTDATE;    //Billing date start, in UTC/GMT format 2012-05-11T00:00:00Z
	params.DESC = args.DESC;    //Profile description - same as billing agreement description
	params.BILLINGPERIOD = args.BILLINGPERIOD;   //Period of time between billings
	params.BILLINGFREQUENCY = args.BILLINGFREQUENCY;    //Frequency of charges
	params.AMT = args.AMT;    //The amount the buyer will pay in a payment period
	params.CURRENCYCODE = args.CURRENCYCODE;    //he currency, e.g. US dollars
	params.COUNTRYCODE = args.COUNTRYCODE;    //The country code, e.g. US
	params.MAXFAILEDPAYMENTS = args.MAXFAILEDPAYMENTS;
...
/**
 * Set addtional options to payoptions.
 * @param  {array}   option  array of documents: {name: "option", value: "value"}
 * @param  {string}   value   [description]
 * @return {Paypal}           [description]
 */
Paypal.prototype.addPayOption = function (options) {
	for (var i in options) {
		this.payOptions[options[i].name] = options[i].value;
	}
	return this;
}
